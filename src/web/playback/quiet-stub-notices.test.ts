...
describe('isStubNotice', () => {
  ...
  it('does not match a real fluidsynth failure', () => {
    ...
    expect(
      isStubNotice('fluidsynth: error: Unable to open file "missing.sf2"')
    ).toBe(false);
    expect(isStubNotice('fluidsynth: error: Failed to allocate voice')).toBe(
      false
    );
  });
});
...
describe('seedMainThreadQuietModule', () => {
  it('installs a printErr that drops stub notices and passes everything else', () => {
    ...
    seedMainThreadQuietModule();
    const printErr = (
      globalThis as unknown as { Module: { printErr: (t: unknown) => void } }
    ).Module.printErr;
    ...
    expect(spy).toHaveBeenCalledWith(
      'fluidsynth: error: something genuinely wrong'
    );
  });

  it('keeps any Module fields an app had already set', () => {
    (globalThis as unknown as { Module?: Record<string, unknown> }).Module = {
      locateFile: 'keep me',
    };
    seedMainThreadQuietModule();
    const mod = (globalThis as unknown as { Module: Record<string, unknown> })
      .Module;
